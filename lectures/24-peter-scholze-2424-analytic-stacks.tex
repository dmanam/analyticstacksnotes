\section{Outlook: Directions for Analytic Geometry}
\label{lec:24}
...
Andreychev \cite{andreychev2023ktheorieadischerraume}, who has
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=1.0,baseline={(0,0)}]
  \draw[->] (-2.2,0) -- (2.2,0) node[right] {$x$};
  % Plot away from zero to avoid division by zero, then insert f(0)=0.
  \draw[thick,domain=-2:-0.02,samples=100,smooth,variable=\x]
    plot ({\x},{exp(-1/(\x*\x))});
  \draw[thick,domain=0.02:2,samples=100,smooth,variable=\x]
    plot ({\x},{exp(-1/(\x*\x))});
  \fill (0,0) circle[radius=0.7pt];
  \node at (1.45,0.98) {$e^{-1/x^2}$};
...
\begin{tikzpicture}[scale=1.0,baseline={(0,0)}]
  ...
  \draw[thick,domain=0.02:2,samples=100,smooth,variable=\x]
    plot ({\x},{exp(-1/(\x*\x))});
  \fill (0,0) circle[radius=0.7pt];
  \node at (1.25,0.98) {$\operatorname{sgn}(x)\,e^{-1/x^2}$};
\end{tikzpicture}
...
complex-geometry notes \cite{scholze2020analytic}), so that
\begin{equation}
