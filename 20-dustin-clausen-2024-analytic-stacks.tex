...
\begin{equation}
\label{eq:20:analytic-affine-line}
...
\begin{equation}
\label{eq:20:gaseous-ring-recall}
...
\left\{\bigl(N\colon\mathbb P^1_R\to[0,\infty]\ \text{a norm on }R,\
q\in\ur R(\ast)\ \text{with}\ N(q)\in(0,1)\bigr)\right\}
\ \xrightarrow{\ \ \sim\ \ }\
\left\{\mathrm{Spec}(R)\xrightarrow{(q,\,N(q))}
\mathrm{Spec}\bigl(\mathbb Z[\widehat q^{\pm1}]_\gasx\bigr)\times(0,1)\right\}
\end{equation}
is an equivalence.
\end{theorem}
